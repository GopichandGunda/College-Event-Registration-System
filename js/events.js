...
const collegeEvents = [
  ...
  { name: 'Hackathon 2026', date: '10 April 2026', time: '9:00 AM - 9:00 PM', venue: 'Digital Learning Centre', fee: 'INR 500', description: 'Build useful ideas with a team, mentors, and a full day to make something real.' },
...
function createEventCard(event) {
  return `<article class="event-card"><div class="event-accent"></div><h3>${event.name}</h3><p>${event.description}</p><div class="event-meta"><div>${event.date} <span>· ${event.time}</span></div><div>${event.venue}</div><div>Entry fee: <span>${event.fee || 'Free'}</span></div></div><a class="button button-primary" href="register.html?event=${encodeURIComponent(event.name)}">Register <span>→</span></a></article>`;
}
